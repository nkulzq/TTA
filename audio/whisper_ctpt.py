...
def compute_atfd_squared(caption_embeds):
    N = caption_embeds.shape[0]
    norm_sq = caption_embeds.pow(2).sum(dim=1, keepdim=True)  # (N, 1)
    dist_matrix = norm_sq + norm_sq.T - 2 * caption_embeds @ caption_embeds.T  # (N, N)
    off_diag_mask = ~torch.eye(N, dtype=torch.bool, device=caption_embeds.device)
    atfd = dist_matrix[off_diag_mask].mean()
    return atfd
...
class CustomSeq2SeqTrainer(Seq2SeqTrainer):
    def compute_loss(self, model, inputs, return_outputs=False, num_items_in_batch=None):
        ...
        loss_etp = -(probs * log_probs).sum(dim=-1).sum()
        text_embeds = outputs["decoder_hidden_states"][-1][:,-1,:].detach().cpu()
        loss_atfd = -compute_atfd_squared(text_embeds)
        loss = loss_etp + loss_atfd
